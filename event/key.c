...
static int	check_play_move(t_param *param, int x, int y)
{
	if (param->map[x][y] == '0')
...
static void	player_move_write(int move_count)
{
	char	write_a;
...
{	
	...
	if (keycode == KEY_W)
		...
		return (0);
	show_image(param->mlx, param->mlx_win, param->map, param->map_info);
